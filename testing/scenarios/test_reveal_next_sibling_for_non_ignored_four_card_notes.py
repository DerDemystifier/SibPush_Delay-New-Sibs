...
def test_process_all_notes_reveals_the_next_sibling_for_non_ignored_four_card_notes() -> None:
    """
    Scenario: Case when a four-card note has two review cards plus two suspended siblings.
    ...
    with temporary_collection() as col:
        ...
        print(
            "Before processing the four-card note: two cards are in review and the two remaining siblings are suspended."
        )
        print_collection_state(col, "Before processing (four-card note)")

        with patched_addon_state(col) as patched_addon:
            patched_addon.process_all_notes(col)

        print(
            "After processing, the next suspended sibling is restored to New, while the last sibling stays suspended."
        )
